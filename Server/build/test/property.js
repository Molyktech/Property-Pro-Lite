...
describe('Property Validation', function () {
  describe('POST', function () {
    ...
    it('should check if property exists', function (done) {
    ...
    it('should check if city empty', function (done) {
      _chai["default"].request(_app["default"]).post('/api/v1/property').set('Authorization', "Bearer ".concat(testToken)).field('price', 'five thousand').field('state', 'lagos').field('city', '').field('address', 'No. 232 off Admiralty way Lekki, Lagos').field('type', '2 bedroom').field('status', 'available').attach('image', _path["default"].join("".concat(__dirname, "/images/apartments.jpg"))).then(function (res) {
        res.should.have.status(422);
        res.body.should.be.an('object');
        res.body.should.have.keys('status', 'error');
        res.body.error.should.be.an('object');
    ...
    it('should check if address empty', function (done) {
      _chai["default"].request(_app["default"]).post('/api/v1/property').set('Authorization', "Bearer ".concat(testToken)).field('price', 'five thousand').field('state', 'lagos').field('city', 'Lagos').field('address', '').field('type', '2 bedroom').field('status', 'available').attach('image', _path["default"].join("".concat(__dirname, "/images/apartments.jpg"))).then(function (res) {
        res.should.have.status(422);
        res.body.should.be.an('object');
        res.body.should.have.keys('status', 'error');
        res.body.error.should.be.an('object');
    ...
    it('should check if status is empty', function (done) {
      _chai["default"].request(_app["default"]).post('/api/v1/property').set('Authorization', "Bearer ".concat(testToken)).field('price', '100000').field('state', 'lagos').field('city', 'Lagos').field('address', 'no, 1 florina street off admiralty way lekki').field('type', '2 bedroom').field('status', '').attach('image', _path["default"].join("".concat(__dirname, "/images/apartments.jpg"))).then(function (res) {
        res.should.have.status(422);
        res.body.should.be.an('object');
        res.body.should.have.keys('status', 'error');
        res.body.error.should.be.an('object');
    ...
    it('should check if status is a string', function (done) {
      _chai["default"].request(_app["default"]).post('/api/v1/property').set('Authorization', "Bearer ".concat(testToken)).field('price', '100000').field('state', 'lagos').field('city', 'Lagos').field('address', 'no, 1 florina street off admiralty way lekki').field('type', '2 bedroom').field('status', 333).attach('image', _path["default"].join("".concat(__dirname, "/images/apartments.jpg"))).then(function (res) {
        res.should.have.status(422);
        res.body.should.be.an('object');
        res.body.should.have.keys('status', 'error');
        res.body.error.should.be.an('object');
